...
export default function StacksSection() {
  return (
    <section className="bg-gray-50 py-20 px-6">
      <div className="max-w-7xl mx-auto text-center">
        ...
        <h2 className="text-4xl font-bold mb-4 text-gray-900">Mes Stacks & Outils</h2>
  ...
  {stacks.map((category) => (
    <div key={category.title}>
      <h3 className="font-semibold text-lg mb-4 text-blue-900">{category.title}</h3>
      <ul className="space-y-3">
        {category.items.map((tool) => (
          <li
            key={tool.name}
            className="flex items-center bg-white rounded-md shadow-sm px-3 py-2 hover:shadow-md transition"
          >
            <span className="text-xl text-blue-500 mr-3">{tool.icon}</span>
            <span className="text-base text-gray-900 font-medium">{tool.name}</span>
          </li>
                ))}
              </ul>
            </div>
